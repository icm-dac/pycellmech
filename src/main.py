    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith((".jpg", ".png", ".jpeg", ".tiff", ".tif")):
            ...
            for i, contour in enumerate(contours):
                ...
                spatial_features = get_spatial_shape_features(contour, cx, cy, file_path)

                combined_features = {'image_name': filename, 'contour_number': contour_number}
                combined_features.update({**one_d_features, **geom_features, **poly_features, **spatial_features})
