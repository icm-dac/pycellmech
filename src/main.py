...
from one_dimensional_features import get_one_dimensional_features
from geometric_shape_features import get_geometric_shape_features
from polygonal_shape_features import get_polyognal_shape_features

from one_dimensional_features import plot_original_contour, plot_centroid, plot_complex_coordinates, plot_cdf, plot_area_function, plot_clf, plot_tar
from geometric_shape_features import plot_AMI, plot_ABE, plot_eccentricity, plot_mbr, plot_circularity_ratio, plot_ellipse_features, plot_solidity
from polygonal_shape_features import plot_DTM, plot_PEVD, plot_SM, plot_MPP, plot_KMeans
...
def main():
    binary_image_path = '/path/to/binary/image'
    csv_file_path = '/path/to/output/shape_features.csv'

    binary_image = cv2.imread(binary_image_path, 0)
    
    contour = preprocess_image(binary_image_path)
    ...
    geom_features = get_geometric_shape_features(contour, cx, cy, binary_image_path)
    poly_features = get_polyognal_shape_features(contour, cx, cy, binary_image_path)

    combined_features = {**one_d_features, **geom_features, **poly_features}
    ...
    fig, axs = plt.subplots(4, 7, figsize=(45, 8), constrained_layout=True)
    ...
    # The feature map will illustrate the largest contour (for now - to be updated in later versions)
    ...
    # Flatten contour array for subsequent analyses and plots
    ...
    # Plot KMeans
    km_cluster_centers, km_line_segments = poly_features['km_cc'], poly_features['km_ls']
    plot_KMeans(axs[2, 4], contour_reshape, km_cluster_centers, km_line_segments)
